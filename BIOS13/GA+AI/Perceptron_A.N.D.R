...
CORRECT_OUTPUT <- c(0,0,0,1) # correct output
...
while(!done && Count < 25) {      # the ! is a NOT operator. Task will be solved when done = TRUE
   ...
   if(TOTAL_ERROR == 0) {
      ...
      total_execution_time <- total_execution_time + execution_time_i
   }
   
   average_generation_time <- total_execution_time / 10
   savecount<- savecount+Count
   } 
 
}
average_count <- savecount/10
print(average_count)
